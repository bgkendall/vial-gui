...
def get_label_mod(modifiers, mod_mask):
	return (modifier_dict[mod_mask]["qmk"][0] if modifiers & mod_mask else "")
...
def get_tooltip_mod(modifiers, mod_mask, suffix = ""):
	return (modifier_dict[mod_mask]["text"] + suffix if modifiers & mod_mask else "")
...
for side in side_dict:
	for mod_mask in modifier_combinations:
		...
		osm_code = QK_ONE_SHOT_MOD | side | mod_mask

		if osm_code not in defined_modifiers:
			print_key(osm_code,
					  "OSM({})".format(get_qmk_mods(side, mod_mask)),
					  "OSM\\n{}".format(get_label_mods(side, mod_mask)),
					  "Enable {} for one keypress".format(get_tooltip_mods(side, mod_mask)),
					  masked=False)
...
for side in side_dict:
	for mod_mask in modifier_combinations:
		...
		mod_code = (side | mod_mask) << 8

		if mod_code not in defined_modifiers:
			qmk_code = ""
			for mod in modifier_dict:
				qmk_code += get_qmk_mod(side, mod_mask, mod, suffix = "(")
			qmk_code += "kc"
			for mod in modifier_dict:
			...
			print_key(mod_code,
					  qmk_code,
					  "{}\\n(kc)".format(get_label_mods(side, mod_mask)),
					  get_tooltip_mods(side, mod_mask) + " + kc")
...
for side in side_dict:
	for mod_mask in modifier_combinations:
		...
		mod_tap_code = MT(side | mod_mask)

		if mod_tap_code not in defined_modifiers:
			print_key(mod_tap_code,
					  "MT({}, (kc))".format(get_qmk_mods(side, mod_mask)),
					  "{}_T\\n(kc)".format(get_label_mods(side, mod_mask)),
					  get_tooltip_mods(side, mod_mask) + " when held, kc when tapped")
